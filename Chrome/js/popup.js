...
function saveTableData(){
	...
	chrome.storage.sync.set({'credentials':array},function(){});
...
function saveIPData(){
	...
	chrome.storage.sync.set({'ip':obj},function(){});
}

function loadTableData(){
	var table=document.getElementsByClassName('listTable')[0];
	
	chrome.storage.sync.get({'credentials':[]},function(tobj){
		if(tobj != undefined){
...
function loadIPData(){
	...
	chrome.storage.sync.get('ip',function(tobj){
		if(tobj != undefined){
			var obj=tobj.ip;
			if(obj!=undefined)
			{
